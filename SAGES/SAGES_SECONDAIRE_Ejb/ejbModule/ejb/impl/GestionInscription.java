...
public class GestionInscription implements GestionInscriptionLocal{
	...
	public boolean supprimerPreinscription(String codedossier) {
		...
		try{
			...
			if(pre==null) throw (new RuntimeException("Essai de suppression d'une préinscription non encore enregistrée "));
	...
	public PreInscription recherchePreinscription(String codedossier,String annee) {
		...
		try{
			...
			Repertoire.logWarn("Dossier non trouvé ou ++ trouvés "+codedossier, this.getClass());
			e.printStackTrace();
		}
		return pr;
	}
...
//		}
//		catch(Exception e){
//			Repertoire.logWarn("Dossier Soft non trouvé ou ++ trouvés "+codedossier, this.getClass());
//			
//			return false;
//		}
...
//	}
	...
	private boolean validerPreinsAvecClasse(String codedossier,String codeclasse,String login,String anneeac){
		...
		try{
			...
			PreInscription pr=em2.find(PreInscription.class,codedossier);
			if(pr==null) throw (new RuntimeException("Essai de modification d'une préinscription non encore enregistrée "));
			
			Annee an=em2.find(Annee.class,anneeac);
			if(an==null) throw (new RuntimeException("Essai d'enregitrement de classe pour un dossier en une année indéfinie "));
	...
	private boolean validerPreinsSansClasse(String codedossier){
		...
		try{
			...
			Repertoire.logFatal(" Fatal -- Validation de préinscription", this.getClass(), e);
			if(em2.getTransaction().isActive())
				em2.getTransaction().rollback();
		}finally{
			em2.close();
	...
	public List<String> affecterClasse(List<String> listedossiers, String codeclasse,String annee) {
		...
		for(int i=0;i<listedossiers.size();i++){
			...
			try{
				...
				if(pr==null) throw (new RuntimeException("Essai d'attribution de classe à une préinscription non encore enregistrée "));
				...
				if(an==null)
					throw (new RuntimeException("Année académique d'enregistrement invalide "));
				el.setAnnee(an);
	...
	public List<Eleve> listerEleveRegulierClasse(String codeclasse, String annee) {
		...
		try{
			...
			if(an==null) throw (new RuntimeException("Année académique non trouvée "+codeclasse));
	...
	public boolean attribuerEnseignantPrincipal(String codeenseignant,
		...
		try{
			...
			if(ens==null) throw(new RuntimeException("Aucun  enseignant trouvé"+codeenseignant));
			
			Classe cla = em2.getReference(Classe.class, codeclasse);
			if(cla==null) throw(new RuntimeException("classe non trouvée"+codeclasse));
		...
		if(datedebut==null){
			...
			if(datefin==null){
				...
				try{
					...
					throw new JPAException("Lister Versements enregistrés",e);
				}
			}
			else{
				// date debut nulle, mais date fin valide
				//recherche de tous les versements jusqu'à date fin
				try{
			...
			if(datefin==null){
				...
				//recherche de tous les versements à partir de date debut
				try{
	...
	public String enregistrerDroitScolaire(float montant,String matricule, String typeversement, Date dateVersement,String annee,String login) throws ElementNOtFoundException, MontantInscriptionInsuffisant, EleveDSCompletException, JPAException, DroitsScolairesNonDefinis, TotalVersementExcedantException{
		...
		try{
			...
			if(droitsScolairescomplet(matricule, annee)){
				// cas où l'élève a payé la totalité des frais
	...
	public Versement rechercherVersement(String idversement,String annee) {
		...
		try{
			...
			Repertoire.logWarn("Versement non trouvé ou ++ trouvés "+idversement, this.getClass());
			e.printStackTrace();
		}
		return vers;
	}
